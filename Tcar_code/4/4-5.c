...
void led_task2(INT stackd, void *exinf)
{
    UW sw4;
...
EXPORT INT usermain(void)
{
    ...
    bz_ctsk.itskpri = 10;
    bz_ctsk.stksz   = 1024;
    led2_ctsk.tskatr  = TA_HLNG | TA_RNG3;
    led2_ctsk.task    = led_task2;
    led2_ctsk.itskpri = 10;
    led2_ctsk.stksz   = 1024;

    led_tskid = tk_cre_tsk( &led_ctsk );
    bz_tskid = tk_cre_tsk( &bz_ctsk );
    led2_tskid = tk_cre_tsk( &led2_ctsk );

    tk_sta_tsk(led_tskid, 0);
    tk_sta_tsk(bz_tskid, 0);
    tk_sta_tsk(led2_tskid, 0);
